...
about = '''
#ABOUT
This is an original project, made by TheReal3A!
Imported from my personal project from 2023, from when I was bored.
#FEATURES
- It helps you find formulas of ionic compounds and acids from name, and
even determine chemical equations for you.
- As a bonus, you may get balanced equations!
- I tried to add as many atoms, ions and molecular compounds as possible to the program!
#NOT SUPPORTED
- Displaying charge of ions
- Most redox reactions
- Reactivity series (It assumes that an ion always displaces another, when this is not the case)
- Solubility
- Temperature
- Detecting acids and bases, measuring pH before and after a reaction
- Individual ions and their reactions
- Organic chemistry
- Actual working reactions with water
- ...anything that's not basic chemistry, lol.
#HISTORY
The latest entry is the last update to this program of the version you have.
4/9/2023: Started the project.
...
13/10/2023: Completely revamped the code, now using mainly classes.
...
* And MORE...
4/5/2023: Uploaded onto Khan Academy.
6/5/2023: Fixed a few wordings and fixed hydride bug (it was treating it as a hydrogen cation).
<end>
'''
...
molecules = [
    ('water', 'H2 O', 0, 1),
    ('hydrogen peroxide', 'H2 O2', 0, 1),
    ('carbon dioxide', 'C O2', 0, 2),
    ('ammonia', 'N H3', 0, 2),
    ('sulfur dioxide', 'S O2', 0, 2),
    ('sulfur trioxide', 'S O3', 0, 2),
    ('nitrogen dioxide', 'N O2', 0, 2),
    ('nitrogen oxide', 'N O', 0, 2),
    ('methane', 'C H4', 0, 2),
    ('ozone', 'O3', 0, 2),
    ('chlorine dioxide', 'Cl O2', 0, 2),
    ...
    ('phosphite', 'P O3', -3),
    ('phosphate', 'P O4', -3),
    ('hydrogensulfate', 'H P O4', -2),
    ('dihydrogensulfate', 'H2 P O4', -1),
...
break_up = { #It's meant to be how molecules decompose, but i turned it to a conversion thingy...
    'hydrogen hydride': ['hydrogen'],
    'hydrogen hydroxide': ['water'],
    'hydrogen oxide': ['water'],
    'carbon hydride': ['methane'],
    'carbon hydroxide': ['carbonic acid'],
...
class Atom(Element):
    ...
    def current_name(self):
        ...
        if self.oxidation is not None and self.oxidation != 0 and self.charge != 0:
            ...
            if l.name in break_up:
                for x in break_up[l.name]:
                    if x in moleculies:
                        ...
                        new_products.append(create_compound(x)[1])
            else:
...
class Unit:
    ...
    def convert(self, new_unit = 'mol'):
        ...
        if 'volume' in [self.type, unit_type]: #If volume is involved in the conversion, there will be an assumption.
            print("ASSUMPTION: " + self.reaction.species(self.destination).formula() + " is gaseous (as an ideal gas) and at STP.")
...
def create_atom(name):
    ...
    for i in other_anion_names:
        if name == i:
            return moleculies[other_anion_names[i]]
...
def runEquationFinder():
    try:
        ...
        res = []
        e = input('Reactants (separated with +): ').lower().strip().split('+')
        print(len(e))
        if len(e) == 1:
...
#print("Running Chemical Finder - made by TheReal3A. WARNING: Basic Chemistry.")
print("Welcome to Chemical Finder! If there's any incorrect output or you have a feature suggestion, lemme know in Questions. If an error shows up at any point, please copy-paste your input and the red error text into Questions.")
print(last_update)
while True:
####
    print("Running Chemical Finder - made by TheReal3A. WARNING: Basic Chemistry.")
    if testing_case:
        ...
        print('Do you want to figure out an equation (1), or a compound formula (2)?')
    ...
    if choice == '1':
        ...
        for i in range(len(About)):
            ...
            if i == 0:
                ...
                if i < len(About):
                    proceed = input('Do you want to see ' + a + '? Leave blank if not. ')
                if proceed == '':
                    print('End!')
                    break
                else:
...
#exit() BUG: exit() will not run input commands when rerunning the program. We have to refresh the site.
...
reactants = [{},{}]
def run():
    print('Running Chemical Finder - made by TheReal3A')
    if testing_case:
        choice = '1'
    else:
        print('Do you want to figure out an equation (1), or a compound formula (2)?')
    ...
    if choice == '1':
        ...
        name = input('What is your compound? ').strip().lower()
        if ' ' in name:
            print('Formula: ', u'' + create_compound(name))
        else:
